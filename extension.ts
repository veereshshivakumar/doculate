...
class DoculatePanel {
    ...
    private async _handleMessage(message: any) {
        switch (message.command) {
            ...
            case 'exportToWord':
                ...
                break;
            case 'debugTemplates':
                await this._debugTemplateStorage();
                break;
    ...
    private async _preprocessMarkdownForPreview(content: string): Promise<string> {
        try {
            console.log('Starting Mermaid preprocessing...');
            const lines = content.split('\n');
            let processedLines: string[] = [];
            let inMermaidBlock = false;
            let mermaidCode: string[] = [];
            ...
            console.log(`Mermaid preprocessing completed. Processed ${diagramCount} diagrams.`);
            return processedLines.join('\n');
        } catch (error) {
            throw new Error(`Markdown preprocessing failed for preview: ${error}`);
        }
    }
    ...
    private async _debugTemplateStorage() {
        try {
            const templates = this._context.globalState.get('doculate.templates', []) as Array<{name: string, path: string}>;
            
            // Get VS Code's storage location info
            ...
            const extensionId = `veereshs88.doculate-ai`; // publisher.name format
            ...
            const debugInfo = {
                ...
                storageLocations: {
                    localStorage: storageUri?.fsPath || 'Not available',
                    globalStorage: globalStorageUri?.fsPath || 'Not available'
                },
                actualStoragePaths: {
                    globalState: `C:\\Users\\${require('os').userInfo().username}\\AppData\\Roaming\\Code\\User\\globalStorage\\${extensionId}`,
                    workspaceState: storageUri?.fsPath || 'Workspace-specific storage'
                },
                platformInfo: {
            ...
            const channel = vscode.window.createOutputChannel('Doculate Debug');
            channel.clear();
            channel.appendLine('=== DOCULATE TEMPLATE STORAGE DEBUG ===');
            channel.appendLine('');
            channel.appendLine(`Extension ID: ${debugInfo.extensionId}`);
            channel.appendLine(`Total Templates: ${debugInfo.totalTemplates}`);
            channel.appendLine('');
            
            if (templates.length > 0) {
                channel.appendLine('Templates:');
                templates.forEach((template, index) => {
                    channel.appendLine(`  ${index + 1}. Name: ${template.name}`);
                    channel.appendLine(`     Path: ${template.path}`);
                    channel.appendLine(`     Exists: ${require('fs').existsSync(template.path) ? 'Yes' : 'No'}`);
                    channel.appendLine('');
                });
            } else {
                channel.appendLine('No templates stored.');
            }
            
            channel.appendLine('VS Code Storage Locations:');
            channel.appendLine(`  Local Storage: ${debugInfo.storageLocations.localStorage}`);
            channel.appendLine(`  Global Storage: ${debugInfo.storageLocations.globalStorage}`);
            channel.appendLine('');
            
            channel.appendLine('Expected Storage Paths:');
            channel.appendLine(`  Global State: ${debugInfo.actualStoragePaths.globalState}`);
            channel.appendLine(`  Workspace State: ${debugInfo.actualStoragePaths.workspaceState}`);
            channel.appendLine('');
            
            channel.appendLine('Platform Info:');
            channel.appendLine(`  Platform: ${debugInfo.platformInfo.platform}`);
            channel.appendLine(`  Architecture: ${debugInfo.platformInfo.architecture}`);
            channel.appendLine(`  Username: ${debugInfo.platformInfo.username}`);
            channel.appendLine(`  Node Version: ${debugInfo.platformInfo.nodeVersion}`);
            
            // Check if actual storage directory exists
            const globalStoragePath = debugInfo.actualStoragePaths.globalState;
            if (require('fs').existsSync(globalStoragePath)) {
                channel.appendLine('');
                channel.appendLine('✅ Global storage directory exists!');
                try {
                    const files = require('fs').readdirSync(globalStoragePath);
                    channel.appendLine(`   Files in storage: ${files.join(', ') || 'Empty'}`);
                } catch (error) {
                    channel.appendLine(`   Error reading storage directory: ${error}`);
                }
            } else {
                channel.appendLine('');
                channel.appendLine('❌ Global storage directory does not exist yet.');
                channel.appendLine('   (This is normal if no templates have been added)');
            }
            
            channel.show();
            
            // Also show a user-friendly message
            const templatePaths = templates.map(t => `${t.name}: ${t.path}`).join('\n');
            const message = templates.length > 0 
                ? `Found ${templates.length} template(s):\n\n${templatePaths}\n\nStorage Path: ${globalStoragePath}\n\nDetailed info shown in Output panel.`
                : `No templates are currently stored.\n\nStorage Path: ${globalStoragePath}\n\nDetailed info shown in Output panel.`;
                
            vscode.window.showInformationMessage(message, 'Open Output Panel').then(selection => {
            ...
            vscode.window.showErrorMessage(`Error debugging template storage: ${error}`);
        }
    }
